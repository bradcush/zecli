...
// To use requires setting mod error
// in main.rs as far as I can tell
use crate::error;
...
pub(crate) fn get_db_paths<P: AsRef<Path>>(
    wallet_dir: Option<P>,
) -> (PathBuf, PathBuf) {
    let a = wallet_dir
        .as_ref()
...
{
    // Initialize the block and wallet DBs
    let (db_cache, db_data) = get_db_paths(wallet_dir);
    let mut db_cache =
        FsBlockDb::for_path(db_cache).map_err(error::Error::from)?;
