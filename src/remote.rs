...
const ECC_TESTNET: &[Server<'_>] =
    &[Server::fixed("lightwalletd.testnet.electriccoin.co", 9067)];
...
const ZEC_ROCKS_TESTNET: &[Server<'_>] =
    &[Server::fixed("testnet.zec.rocks", 443)];
...
impl ServerOperator {
    fn servers(&self, network: Network) -> &[Server<'_>] {
        match (self, network) {
            ...
            (ServerOperator::ZecRocks, Network::MainNetwork) => {
                ZEC_ROCKS_MAINNET
            }
            (ServerOperator::ZecRocks, Network::TestNetwork) => {
                ZEC_ROCKS_TESTNET
            }
        }
    }
    ...
    pub(crate) fn pick(&self, network: Network) -> anyhow::Result<&Server<'_>> {
        ...
        match self {
            Servers::Hosted(server_operator) => {
                server_operator.servers(network).first().ok_or(anyhow!(
                    "{:?} doesn't serve {:?}",
                    server_operator,
                    network
                ))
...
impl Server<'_> {
    ...
    fn use_tls(&self) -> bool {
        // Assume that localhost will never have a
        // cert, and require remotes to have one.
    ...
    /// Connects to the server over Tor, unless it
    /// is running on localhost without HTTPS.
